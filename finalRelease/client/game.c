...
void collisionDetect(GameState* game) {
	...
	if (getTankPositionX((*game).tanks[0]) <= 5)
	{
		setTankX((*game).tanks[0], 5);
	}
	if (getTankPositionY((*game).tanks[0]) <= 5)
	{
		setTankY((*game).tanks[0], 5);
	}
	if (getTankPositionX((*game).tanks[0]) >= 960 - 48 - 5)
	{
		setTankX((*game).tanks[0], 960 - 48 - 5);
	}
	if (getTankPositionY((*game).tanks[0]) >= 720 - 48 - 5)
	{
		setTankY((*game).tanks[0], 720 - 48 - 5);
	}
...
void handleScore(GameState *game, int* gameOver, SDL_Window* win) {
	...
	if (deadCount >= 3 && (*game).haveUpdated) {
		...
		if (score[0] == 3 || score[1] == 3 || score[2] == 3 || score[3] == 3) *gameOver = 1;
		else {
